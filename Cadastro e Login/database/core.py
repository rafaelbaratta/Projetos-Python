...
class Core:
    ...
    def register_user(register, show):
        ...
        if val.has_blank_space_register(username, email, password, password_repeated):
            messagebox.showwarning("Atenção!", "Certifique-se de preencher TODOS os campos!")
            return
        
        if len(password) < 6:
            messagebox.showwarning("Atenção!", "A senha do usuário deve possuir no mínimo 6 dígitos!")
            return
        ...
        Core.clear_entries(register.name_entry, register.email_entry, register.password_entry, register.password_repeat_entry)
