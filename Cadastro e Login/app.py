...
class App(ctk.CTk):
    ...
    def configure_buttons(self):
        ...
        login_screen.login_button.configure(command=lambda: self.core.login_user(login_screen, main_screen, app.show_screen))
        login_screen.register_button.configure(command=lambda: (self.core.clear_entries(login_screen.email_entry, login_screen.password_entry), self.show_screen("register")))

        register_screen = self.screens["register"]
        register_screen.login_button.configure(command=lambda: (self.core.clear_entries(register_screen.name_entry, register_screen.email_entry, register_screen.password_entry, register_screen.password_repeat_entry), self.show_screen("login")))
        register_screen.register_button.configure(command=lambda: self.core.register_user(register_screen, app.show_screen))
